...
t(c(37,38,39))
c(37,38,39)
respca2=PCA(GenT,scale.unit=T,graph=F)
respca2$eig
fviz_eig(respca2)
...
fviz_pca_ind(respca2,col.ind="contrib",geom=c("text"))

fviz_pca_var(respca2,habillage=Gen$ExpT1,axes=c(1,2))#faire un interpretation pour chaque Exp (T1,T2,T3)
fviz_pca_var(respca2,habillage=Gen$ExpT2,axes=c(1,2))
fviz_pca_var(respca2,habillage=Gen$ExpT3,axes=c(1,2))
#plot(respca,choix="varcor",axes=c(1,5))
````

```{r}
Kmax<-15
reskmeanscl<-matrix(0,nrow=nrow(GenT),ncol=Kmax-1)
Iintra<-NULL
for (k in 2:Kmax){
  resaux<-kmeans(GenT,center=k)
...
set.seed(123)
kmeans_res <- kmeans(GenT, centers = 3, nstart = 25)
fviz_cluster(kmeans_res, data = GenT)
kmeans_res <- kmeans(GenT, centers = 5, nstart = 25)
fviz_cluster(kmeans_res, data = GenT)

Classif<-kmeans(GenT,3,nstart=25)
aux<-silhouette(Classif$cl, daisy(GenT))
fviz_silhouette(aux)+theme(plot.title = element_text(size =9))

Silhou<-NULL
for (k in 2:Kmax){
   aux<-silhouette(reskmeanscl[,k-1], daisy(GenT))
...
aux<-silhouette(kmeans_res$cluster, daisy(GenT))
fviz_silhouette(aux)+
  theme(plot.title = element_text(size =9))
rm(df,Silhou,aux)

Classif<-kmeans(GenT,5,nstart=25)
aux<-silhouette(Classif$cl, daisy(GenT))
fviz_silhouette(aux)+theme(plot.title = element_text(size =9))
